...
if (args.Length != 1)
    throw new ArgumentException("assembly path required");
...
if (type.Methods.Any(m => m.Name == "InitializeAsyncDeferredCore"))
...
for (var index = 0; index < original.Body.Instructions.Count; index++)
{
    ...
    var destination = clone.Body.Instructions[index];
    destination.Operand = source.Operand switch
    {
