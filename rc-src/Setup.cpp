...
static fs::path WriteSetupLog(const std::wstring& message) noexcept {
    try {
        const fs::path setupDir = ExePath().parent_path();
        const fs::path dir = setupDir / L"SyntexSetup-Log";
        std::error_code ec;
        fs::create_directories(dir, ec);
        if (ec) return {};
        const fs::path path = dir / (L"SyntexSetup-Error-" + Timestamp() + L"-" + std::to_wstring(GetCurrentProcessId()) + L".log");
        std::ofstream f(path, std::ios::binary | std::ios::trunc);
        if (!f) return {};
        const std::wstring body = L"Syntex Launcher Setup 0.16.6 RC1\r\nZeit: " + Timestamp() +
            L"\r\nSetup-Verzeichnis: " + setupDir.wstring() +
            L"\r\nProzess-ID: " + std::to_wstring(GetCurrentProcessId()) +
            L"\r\n\r\nFehler:\r\n" + message + L"\r\n";
        const auto u8 = Utf8(body);
...
static LONG WINAPI TopLevelCrashFilter(EXCEPTION_POINTERS* info) {
    ...
    if (info && info->ExceptionRecord) {
        wchar_t b[32]{};
        swprintf_s(b, L"0x%08X", info->ExceptionRecord->ExceptionCode);
        msg += L"\r\nException-Code: ";
        msg += b;
    }
    WriteSetupLog(msg);
...
static std::wstring DefaultInstallPath() {
    const auto registered = ExistingInstallPath();
...
static bool ValidateWritableRoot(const fs::path& root, std::wstring& error) {
    ...
    const fs::path a = root / L".syntex-write-test.tmp";
    const fs::path b = root / L".syntex-write-test-renamed.tmp";
    {
...
static bool HttpDownload(const std::wstring& url, const fs::path& target, unsigned long long expectedSize, std::wstring& error, bool reportProgress) {
    ...
    if (!WinHttpCrackUrl(url.c_str(), 0, 0, &parts) || parts.nScheme != INTERNET_SCHEME_HTTPS) {
        error = L"Ungültige oder unsichere Download-Adresse.";
        return false;
    }
    ...
    if (!connect || !request || !WinHttpSendRequest(request, WINHTTP_NO_ADDITIONAL_HEADERS, 0, WINHTTP_NO_REQUEST_DATA, 0, 0, 0) || !WinHttpReceiveResponse(request, nullptr)) {
        ...
        error = L"Launcher-Download fehlgeschlagen.";
        return false;
    }
    ...
    if (!f) {
        error = L"Temporäre Download-Datei konnte nicht angelegt werden.";
        WinHttpCloseHandle(request);
        WinHttpCloseHandle(connect);
        WinHttpCloseHandle(session);
        return false;
    ...
    while (WinHttpQueryDataAvailable(request, &available) && available > 0) {
        ...
        if (!WinHttpReadData(request, buffer.data(), available, &read)) {
            error = L"Launcher-Download wurde unterbrochen.";
            f.close();
            WinHttpCloseHandle(request);
            WinHttpCloseHandle(connect);
            WinHttpCloseHandle(session);
        ...
        if (reportProgress && expectedSize > 0) {
            const int raw = static_cast<int>(std::min<unsigned long long>(100, total * 100ULL / expectedSize));
    ...
    if (expectedSize && total != expectedSize) {
        error = L"Die heruntergeladene Dateigröße stimmt nicht mit dem geprüften Paket überein.";
        return false;
    }
    return true;
}
...
static bool Sha256File(const fs::path& path, std::wstring& hex, std::wstring& error) {
    ...
    if (!f) {
        ...
        BCryptCloseAlgorithmProvider(alg, 0);
        error = L"Download-Datei konnte nicht geprüft werden.";
        return false;
    }
    std::vector<UCHAR> buffer(1024 * 1024);
    while (f) {
        f.read(reinterpret_cast<char*>(buffer.data()), static_cast<std::streamsize>(buffer.size()));
        const ULONG n = static_cast<ULONG>(f.gcount());
        if (n && BCryptHashData(hash, buffer.data(), n, 0) < 0) {
            BCryptDestroyHash(hash);
            BCryptCloseAlgorithmProvider(alg, 0);
...
static bool RunProcess(const std::wstring& command, const fs::path& cwd, DWORD& exitCode, std::wstring& error, DWORD timeout = INFINITE) {
    ...
    if (!CreateProcessW(nullptr, mutableCmd.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr,
        ...
        error = L"Windows konnte ein benötigtes Hilfsprogramm nicht starten. Fehler " + std::to_wstring(GetLastError()) + L".";
        return false;
    }
    ...
    if (wait == WAIT_TIMEOUT) {
        ...
        error = L"Ein Installationsschritt hat das Zeitlimit überschritten.";
        return false;
    }
    if (wait != WAIT_OBJECT_0) {
        CloseHandle(pi.hProcess);
        error = L"Ein Installationsschritt konnte nicht überwacht werden.";
        return false;
    }
...
static bool ExtractZip(const fs::path& zip, const fs::path& target, std::wstring& error) {
    ...
    if (!GetSystemDirectoryW(systemDir, MAX_PATH)) { error = L"Windows-Systemordner konnte nicht ermittelt werden."; return false; }
    ...
    if (ec) { error = L"Temporärer Installationsbereich konnte nicht erstellt werden."; return false; }
    DWORD code = 0;
...
static bool ValidateStage(const fs::path& stage, std::wstring& error) {
    const fs::path required[] = {
        ...
        stage / L"app" / L"SyntexLauncher.App.exe",
        stage / L"app" / L"SyntexUninstall.exe"
    };
    for (const auto& file : required) {
        if (!fs::exists(file)) {
            error = L"Installationspaket ist unvollständig: " + file.filename().wstring() + L" fehlt.";
            return false;
        }
    }
    return true;
...
static bool CopyStage(const fs::path& stage, const fs::path& root, std::wstring& error) {
    ...
    for (const auto& entry : fs::directory_iterator(stage, ec)) {
        ...
        if (ec) {
            error = L"Neue Programmdateien konnten nicht vollständig installiert werden: " + entry.path().filename().wstring();
            return false;
        }
    }
    return true;
...
static bool CreateShortcut(const fs::path& target, const fs::path& shortcut, std::wstring& error) {
    ...
    if (FAILED(hr) || !link) {
        ...
        error = L"Verknüpfung konnte nicht erstellt werden.";
        return false;
    }
    link->SetPath(target.c_str());
    link->SetWorkingDirectory(target.parent_path().c_str());
    link->SetDescription(L"Syntex Launcher");
...
static bool RegisterUninstaller(const fs::path& root, std::wstring& error) {
    const fs::path uninstaller = root / L"app" / L"SyntexUninstall.exe";
    HKEY key = nullptr;
    DWORD disp = 0;
    const LONG rc = RegCreateKeyExW(HKEY_CURRENT_USER, kUninstallKey, 0, nullptr, 0, KEY_SET_VALUE, nullptr, &key, &disp);
    if (rc != ERROR_SUCCESS || !key) {
        error = L"Windows-Deinstallation konnte nicht registriert werden (Fehler " + std::to_wstring(rc) + L").";
        return false;
    }
    DWORD noModify = 1, noRepair = 1;
    DWORD estimatedKb = static_cast<DWORD>(std::min<unsigned long long>(PAYLOAD_SIZE / 1024ULL, 0xFFFFFFFFULL));
    SYSTEMTIME st{};
    GetLocalTime(&st);
    wchar_t date[16]{};
    swprintf_s(date, L"%04u%02u%02u", st.wYear, st.wMonth, st.wDay);
    const bool ok = SetRegString(key, L"DisplayName", L"Syntex Launcher") &&
        SetRegString(key, L"DisplayVersion", L"0.16.6") &&
        SetRegString(key, L"Publisher", L"Syntex Launcher Project") &&
        SetRegString(key, L"InstallLocation", root.wstring()) &&
        SetRegString(key, L"DisplayIcon", (root / L"SyntexLauncher.exe").wstring() + L",0") &&
        SetRegString(key, L"UninstallString", Quote(uninstaller.wstring())) &&
...
static bool LaunchVerify(const fs::path& root, bool ciMode, std::wstring& error) {
    ...
    if (code != 0) {
        error = L"Syntex Launcher konnte nach der Installation kein funktionsfähiges sichtbares Launcherfenster bestätigen. Startwächter-Exitcode: " +
            std::to_wstring(code) + L". Prüfe storage\\logs\\launcher-startup.";
        return false;
    }
    return true;
}
...
static bool InstallCore(const fs::path& root, bool ciMode, bool reportProgress, bool& wasRepair, std::wstring& error) {
    const fs::path temp = fs::temp_directory_path() / (L"SyntexLauncher-0166RC-" + std::to_wstring(GetCurrentProcessId()) + L".zip");
    const fs::path stage = fs::temp_directory_path() / (L"SyntexLauncher-0166RC-stage-" + std::to_wstring(GetCurrentProcessId()));
    std::error_code ec;
    fs::remove(temp, ec);
    ec.clear();
    ...
    SyntexSetupRepair::Transaction transaction;
    ...
    auto rollback = [&]() {
        if (!transactionPrepared) return;
        std::wstring rollbackError;
        progress(98, L"Fehler erkannt – vorherige Programmversion wird automatisch wiederhergestellt …");
        if (!SyntexSetupRepair::Rollback(root, transaction, rollbackError) && !rollbackError.empty()) {
            if (!error.empty()) error += L"\r\n\r\n";
            error += L"Zusätzlicher Rollback-Fehler: " + rollbackError;
        }
        transactionPrepared = false;
    };

    progress(3, L"Installationsordner wird geprüft …");
    if (!ValidateWritableRoot(root, error)) { cleanup(); return false; }

    progress(8, L"Geprüftes Reparatur-/Installationspaket wird vorbereitet …");
    if (!HttpDownload(PAYLOAD_URL, temp, PAYLOAD_SIZE, error, reportProgress)) { cleanup(); return false; }

    progress(73, L"Download wird mit SHA-256 geprüft …");
    std::wstring actual;
    if (!Sha256File(temp, actual, error)) { cleanup(); return false; }
    if (_wcsicmp(actual.c_str(), PAYLOAD_SHA256) != 0) {
        error = L"Sicherheitsprüfung fehlgeschlagen: SHA-256 stimmt nicht mit dem veröffentlichten Paket überein.";
        cleanup();
        return false;
    }

    progress(80, L"Neue Programmdateien werden separat entpackt und geprüft …");
    if (!ExtractZip(temp, stage, error) || !ValidateStage(stage, error)) { cleanup(); return false; }
    ...
    wasRepair = transaction.repair;

    progress(90, wasRepair ? L"Neue Programmdateien werden installiert …" : L"Syntex Launcher wird installiert …");
    if (!CopyStage(stage, root, error)) {
        rollback();
        cleanup();
        return false;
    }

    progress(96, L"Syntex Launcher wird gestartet und auf ein sichtbares Fenster geprüft …");
    if (!LaunchVerify(root, ciMode, error)) {
        rollback();
        cleanup();
        return false;
    }

    progress(98, L"Geprüfte Installation wird abgeschlossen …");
    if (!SyntexSetupRepair::Commit(transaction, error)) {
        // Commit only removes the old backup after a successful launcher health check.
        // If this cleanup fails, keep the backup and report the condition instead of hiding it.
        transactionPrepared = false;
        cleanup();
        return false;
    }
    ...
    if (!ciMode) {
        progress(99, L"Verknüpfungen und Windows-Deinstallation werden eingerichtet …");
...
static bool RepairSelfTest() {
    return SyntexSetupRepair::RunFilesystemSelfTest();
}

static std::wstring BrowseForFolder(HWND owner) {
    BROWSEINFOW bi{};
    bi.hwndOwner = owner;
    bi.lpszTitle = L"Installationsordner für Syntex Launcher auswählen";
...
static void Worker(std::wstring rootText) {
    bool repair = false;
    std::wstring error;
    const bool ok = InstallCore(fs::path(rootText), false, true, repair, error);
    if (ok) {
        PostMessageW(gWindow, WM_INSTALL_DONE, repair ? 2 : 1, 0);
    } else {
        const auto log = WriteSetupLog(error);
...
static LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE: {
            ...
            const std::wstring initial = DefaultInstallPath();
            ...
            const bool existing = fs::exists(fs::path(initial) / L"SyntexLauncher.exe") || fs::exists(fs::path(initial) / L"app") || fs::exists(fs::path(initial) / L"storage") || fs::exists(fs::path(initial) / L"data");
            ...
            gProgress = CreateWindowExW(0, PROGRESS_CLASSW, nullptr, WS_CHILD | WS_VISIBLE | PBS_SMOOTH, 24, 187, 470, 20, hwnd, reinterpret_cast<HMENU>(1004), nullptr, nullptr);
            ...
            CreateWindowW(L"BUTTON", L"Abbrechen", WS_CHILD | WS_VISIBLE, 462, 229, 100, 32, hwnd, reinterpret_cast<HMENU>(IDCANCEL), nullptr, nullptr);
            if (gProgress) {
        ...
        case WM_COMMAND: {
            ...
            if (id == 1002) {
                const auto p = BrowseForFolder(hwnd);
                if (!p.empty()) SetWindowTextW(gPathEdit, p.c_str());
                return 0;
            }
            if (id == 1003) {
                wchar_t b[4096]{};
                GetWindowTextW(gPathEdit, b, static_cast<int>(std::size(b)));
                if (!wcslen(b)) {
                    MessageBoxW(hwnd, L"Bitte einen Installationsordner auswählen.", L"Syntex Launcher Setup", MB_OK | MB_ICONWARNING);
                    return 0;
                }
                EnableWindow(gInstall, FALSE);
                EnableWindow(gBrowse, FALSE);
                EnableWindow(gPathEdit, FALSE);
                SendMessageW(gProgress, PBM_SETPOS, 0, 0);
                SetWindowTextW(gPercent, L"0 %");
                std::thread(Worker, std::wstring(b)).detach();
                return 0;
            }
        ...
        case WM_PROGRESS_UPDATE: {
            const int p = static_cast<int>(wParam);
            auto* text = reinterpret_cast<std::wstring*>(lParam);
            if (text) { SetWindowTextW(gStatus, text->c_str()); delete text; }
            SendMessageW(gProgress, PBM_SETPOS, p, 0);
            const std::wstring pct = std::to_wstring(p) + L" %";
            SetWindowTextW(gPercent, pct.c_str());
            return 0;
        }
        case WM_INSTALL_DONE: {
            if (wParam == 1 || wParam == 2) {
                SendMessageW(gProgress, PBM_SETPOS, 100, 0);
                SetWindowTextW(gPercent, L"100 %");
                const bool repair = wParam == 2;
                SetWindowTextW(gStatus, repair ? L"Reparatur/Aktualisierung abgeschlossen." : L"Installation abgeschlossen.");
                MessageBoxW(hwnd,
                    repair ? L"Die vorhandene Syntex-Installation wurde transaktional repariert/aktualisiert und ein sichtbares Launcherfenster wurde bestätigt."
                           : L"Syntex Launcher wurde installiert und ein sichtbares Launcherfenster wurde bestätigt.",
                    L"Syntex Launcher Setup", MB_OK | MB_ICONINFORMATION);
                DestroyWindow(hwnd);
            } else {
                auto* text = reinterpret_cast<std::wstring*>(lParam);
                const std::wstring message = text ? *text : L"Unbekannter Installationsfehler.";
                delete text;
                SetWindowTextW(gStatus, L"Installation/Reparatur fehlgeschlagen.");
                EnableWindow(gInstall, TRUE);
                EnableWindow(gBrowse, TRUE);
                EnableWindow(gPathEdit, TRUE);
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    ...
    if (argv && argc == 2 && _wcsicmp(argv[1], L"--self-test") == 0) {
        LocalFree(argv);
        return (wcslen(PAYLOAD_SHA256) == 64 && PAYLOAD_SIZE > 0 && wcsncmp(PAYLOAD_URL, L"https://", 8) == 0) ? 0 : 2;
    }
    if (argv && argc == 2 && _wcsicmp(argv[1], L"--log-self-test") == 0) {
        const auto p = WriteSetupLog(L"SyntexSetup-Log CI self-test");
        LocalFree(argv);
        return p.empty() ? 3 : 0;
    }
    if (argv && argc == 2 && _wcsicmp(argv[1], L"--repair-self-test") == 0) {
        const bool ok = RepairSelfTest();
        LocalFree(argv);
        return ok ? 0 : 4;
    }
    ...
    gWindow = CreateWindowExW(0, kClassName, L"Syntex Launcher Setup 0.16.6",
